    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"'DM Sans',sans-serif", overflowX:"hidden" }}>
      ...
      <section className="signal-grid" style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", padding:isMobile?"90px 1.2rem 60px":"120px 2rem 80px" }}>
        ...
        <div style={{ maxWidth:820, margin:"0 auto", textAlign:"center", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
            ...
            <Link to="/candidature/telecom" style={{ background:"rgba(14,165,233,.08)", border:"1.5px solid rgba(56,189,248,.4)", color:C.skyL, padding:"15px 32px", borderRadius:14, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:".95rem", display:"flex", alignItems:"center", gap:9, textDecoration:"none", transition:"all .3s" }}
      ...
      <section style={{ padding:isMobile?"56px 1.2rem":"88px 2rem", maxWidth:1060, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":isTablet?"1fr 1fr":"repeat(3,1fr)", gap:20 }}>
          {[
            ...
            const [ref, inView] = useInView();
            return (
              <div key={i} ref={ref} style={{ background:C.card, border:`1px solid ${C.border}`, borderTop:`2px solid ${color}`, borderRadius:20, padding:28, transition:`all .6s ease ${i*100}ms`, opacity:inView?1:0, transform:inView?"translateY(0)":"translateY(30px)" }}
      ...
      <section id="solution" style={{ padding:isMobile?"56px 1.2rem":"88px 2rem", maxWidth:1060, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)", gap:20, marginBottom:40 }}>
          {[
            { icon:Shield, accent:"#0EA5E9", sub:"Vérification #1", title:"Certifications validées", desc:"Certification OFAC drone (A1/A2/A3) en cours de validité. Niveau ORNI testé avec les valeurs limites réelles par bande de fréquence. Références opérateurs vérifiées." },
            { icon:MapPin,  accent:C.cyan,   sub:"Vérification #2", title:"Réseau local suisse", desc:"Notre réseau couvre Vaud, Genève, Berne, Zurich et la Suisse alémanique. Profils locaux connaissant les procédures cantonales — pas de formation ORNI depuis zéro." },
            { icon:Waves,  accent:C.gold,    sub:"Vérification #3", title:"Validation technique", desc:"Un spécialiste RNI qui ne connaît pas 4 V/m (900 MHz) de mémoire n'est pas senior. On pose les questions que votre DRH ne pose pas." },
          ].map(({ icon:Icon, accent, sub, title, desc }, i) => {
            const [ref, inView] = useInView();
            const [hov, setHov] = useState(false);
            return (
              <div key={i} ref={ref} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
                style={{ background:hov?`rgba(14,165,233,.06)`:C.card, border:hov?`1px solid rgba(56,189,248,.3)`:`1px solid ${C.border}`, borderRadius:20, padding:28, transition:`all .5s ease ${i*100}ms`, opacity:inView?1:0, transform:inView?"translateY(0)":"translateY(25px)", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:1060, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          ...
          <div style={{ display:isMobile?"none":"flex", gap:16 }}>
            {[["Confidentialité","/privacy"],["CGU","/cgu"],["Contact","/contact"]].map(([label,to]) => (
              <Link key={to} to={to} style={{ color:C.subtle, fontSize:".72rem", textDecoration:"none", transition:"color .2s" }} onMouseEnter={e=>e.target.style.color=C.muted} onMouseLeave={e=>e.target.style.color=C.subtle}>{label}</Link>
            ))}
          </div>
        </div>
      </footer>
